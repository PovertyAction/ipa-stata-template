...
#   3. Find output: reports/analysis_report_final.docx
#   
#
format:
...
def latex_table_to_html(tex_content):
    ...
    for row in rows:
        ...
        # Check for rules
        if '\\toprule' in row or '\\midrule' in row or '\\bottomrule' in row:
            continue
...
def latex_table_to_markdown(tex_content):
    ...
    table_data = []
    for row in rows:
        # Skip tabular begin/end and rules
        if any(x in row for x in ['begin{tabular}', 'end{tabular}', '\\toprule', '\\midrule', '\\bottomrule']):
            continue
...
print(f"Tables exist: {tables_exist}, Figures exist: {figures_exist}")
```
