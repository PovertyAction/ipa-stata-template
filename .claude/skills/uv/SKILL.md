...
pyenv, virtualenv, and more while being 10-100x faster. The skill focuses particularly
on `uv run` for executing code within isolated project environments.
...
**Key behavior:** `uv run` automatically ensures the project environment is current
before executing the command. It installs the project into `.venv` and keeps it
...
**Important:** When a script has inline metadata, `uv run` uses ONLY those dependencies
and ignores the project's dependencies, even when run inside a project directory.
